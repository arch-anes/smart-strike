...
export class OAuth2Service {
    ...
    constructor() {
        ...
        this.server.grant(
            grant.code((client, redirectUri, user, ares, done) => {
                const code = uuidv4();
        ...
        this.server.grant(
            grant.token((client, user, ares, done) => {
        ...
        this.server.exchange(
            exchange.code((client, code, redirectUri, done) => {
        ...
        this.server.exchange(
            exchange.password((client, username, password, scope, done) => {
                // Validate the client
        ...
        this.server.exchange(
            exchange.clientCredentials((client, scope, done) => {
                // Validate the client
                db.clients.findByClientId(client.clientId, (error, localClient) => {
                    if (error) return done(error);
                    if (!localClient) return done(null, false);
        ...
        this.server.exchange(
            exchange.refreshToken((client, refreshToken, scope, done) => {
...
module.exports.token = [passport.authenticate(['basic', 'oauth2-client-password'], { session: false }), server.token(), server.errorHandler()];
